...
&emsp;&emsp;在随后的几年中，神经网络引起了许多研究人员的关注。 1965年，由Ivakhnenko和其他人推出了一种神经网络，该方法基于Rosenblatt著名的感知机（Perceptron），通过一种称为数据分组处理方法（**Group Method of Data Handling，GMDH**）的方法进行训练。 后来，在1979年，Fukushima引入了神经认知机（Neocognitron），这为最着名的深度模型 - 卷积神经网络之一奠定了基础。 与始终采用一维输入的感知器不同，神经认知机能够使用卷积操作处理2D输入。  
&emsp;&emsp;人工神经网络开始使用误差反向传播算法，通过计算层与层之间的Jacobian矩阵来优化网络参数。 此外，消失梯度的问题严格限制了神经网络的潜在层数（深度）。 靠近输入的层的梯度非常小，被称为**梯度消失现象（vanishing gradients phenomenon）**。 这是由于使用链式法则来计算较低层权重的梯度（Jacobian矩阵）。 这种现象又限制了经典神经网络的看似合理的最大深度。  
&emsp;&emsp;随后在2006年， _预训练（pretraining）_ 深度神经网络被发现，通过对每一层进行最小化重建误差（通过尝试将输入映射到较低维度，然后将其重建回原始维度），为神经网络提供了不错的初始参数值;这符合从输出层到输入层的连续梯度。这基本上可以使神经网络模型具有更多层而没有消失梯度的不利影响。此外，这些更深层次的模型能够在许多任务中超越传统的机器学习模型，主要是计算机视觉（例如，MNIST手写数字数据集的测试精度）。随着这一突破，深度学习成为机器学习社区的流行语。  
&emsp;&emsp;在2012年，AlexNet（由Alex Krizhevsky（ http://www.cs.toronto.edu/~kriz/ ），Ilya Sutskever（ http://www.cs.toronto.edu/~lya/ ） 和Geoff Hinton创建的深度卷积神经网络）赢得2012年**大规模视觉识别挑战赛（Large Scale Visual Recognition Challenge，LSVRC）**，误差从之前的最佳值下降了10％。在此期间，语音识别也取得了进展，其中深度神经网络取得了语音识别准确度的最高水平。此外，人们开始意识到与 **中央处理单元（Central Processing Units，CPU）** 相比， **图形处理单元（Graphical Processing Units，GPU）** 能够实现更多的并行性，可以更快地训练更大更深的网络。    
&emsp;&emsp;通过更好的模型初始化技术（例如，Xavier初始化）进一步改进了深度模型，使得耗时的预训练变得多余。此外，引入了更好的非线性激活函数，例如 **修正线性函数（Rectified Linear Units，ReLUs）** ，这减轻了深层模型中消失梯度的不良影响。更好的优化（或学习）技术，如Adam，自动调整我们在神经网络模型中的数百万参数中每个参数的学习率，这项技术突破了许多机器学习领域的最高水平，如对象分类和语音识别。这些进步还允许神经网络模型具有更大规模隐藏层。与其他机器学习模型相比，增加隐藏层数（即使神经网络更深）的能力是神经网络模型显着更好性能的主要源泉之一。此外，更好的中间正则化技术，例如批量归一化层，已经在许多任务上提高了深度网络的性能。   
&emsp;&emsp;后来，引入了更深层的模型，如ResNets，Highway Nets和Ladder Nets，它们有数百层和数十亿个参数。得益于各种经验和理论上的令人振奋技术，建立如此庞大层数的神经网络模型成为了可能。例如，ResNets使用快捷通道连接相距很远的层，正如前面说的那样，这种方法可以减少层到层的梯度消失。  
### 深度学习和NLP的现状
&emsp;&emsp;自2000年初开始以来，许多不同的深度模型已经开始相继出现，即使它们有相似之处，像都使用输入和参数的非线性变换，但是细节也会有很大差异。例如， **卷积神经网络（Convolution Neural Network，CNN）** 可以原样从二维数据（例如，RGB图像）中学习，而多层感知器模型需要将输入展开为一维向量，从而导致重要的空间信息的丢失。    
&emsp;&emsp;处理文本时，作为对文本最直观的解释之一是将其视为字符序列，学习模型应该能够进行时间序列建模，因此需要记忆过去。要理解这一点，请考虑语言建模任务;单词cat的下一个单词应该与单词climb的下一个单词不同。一种包含该能力的流行模型是 **递归神经网络（Recurrent Neural Network，RNN）** 。我们将在 _第6章“回归神经网络”_ 中看到RNN如何通过交互式练习来实现这一点。   
&emsp;&emsp;应该注意，记忆不是学习模型固有的微不足道的操作。相反，我们应该更加严谨的设计持久记忆的方法。此外，术语 _记忆_ 不应与仅依赖当前输入的非序列深度网络的学习权重相混淆，序列模型（例如，RNN）将同时查看学习权重和序列的前一个元素来预测下一个输出。  
&emsp;&emsp;RNN的一个突出的缺点是不能保留超过几个（大约7个）时间步长之外的信息，因此缺乏长期记忆。 **长短时记忆网络（Long Short-Term Memory，LSTM）** 是可封装长期记忆的RNN扩展版本。因此，LSTM通常优于标准RNN。我们将在 _第7章长期短期记忆网络_ 中深入探讨，以便更好地理解它们。  
...
&emsp;&emsp;FCNN的目标是将输入（例如，图像或句子）映射到特定标签或标注（例如，图像的对象类别），其通过对输入 _x_ 使用诸如 _h = sigma（W * x + b）_ 之类的变换来计算 _h_（ _x_ 的隐式表示）实现的。这里，W和b分别是FCNN的权重和偏差，sigma是S形激活函数。接下来，将分类器（例如，softmax分类器）放置在FCNN之上，使得能够利用隐藏层中的学习特征来对输入进行分类。分类器，基本上是FCNN的一部分，而另一个隐藏层具有一些权重Ws和偏差bs。此外，我们可以计算FCNN的最终输出，输出= softmax（Ws * h + bs）。例如，softmax分类器提供分类器层输出的分数的归一化表示;标签被认为是具有最高softmax值的输出节点。然后，通过这个，我们可以定义一个分类损失，计算预测输出标签和实际输出标签之间的差异，这种损失函数的一个例子是均方损失。如果你不理解损失函数的实际复杂性，你不必担心，我们将在后面的章节中讨论。接下来，使用标准随机优化器（例如，随机梯度下降）来优化神经网络参数W，b，Ws和bs，以减少所有输入的分类损失。图1.5描述了本段中针对三层FCNN解释的过程，我们将在第3章，Word2vec-学习单词嵌入中逐步介绍如何将这种模型用于NLP任务的细节。
&emsp;&emsp;
&emsp;&emsp;
&emsp;&emsp;
&emsp;&emsp;
